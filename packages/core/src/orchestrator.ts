...
export const BaseOrchestrator: Orchestrator = (opts: OrchestratorOpts) => {
	...
	return async () => {
		...
		console.log(`Waiting for ${concurrency - runners} jobs`);
		const { runs, info } = await store.poll(concurrency - runners);
		...
		for (const run of runs) {
			...
			const { promise, amount } = await runner.run();
			runners += amount;
			running.push(WrappedPromise(promise.then(() => { runners -= amount })));
		}
